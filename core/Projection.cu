...
int projection_component_batch_capacity(
    ...
    int number_pairs)
{
    // The packed values exist once in GVVAmplitude's managed scratch and
    ...
    double minimum)
{
    ...
    const GVVCompiledModel& model = amplitude.Model();
    ...
    const double* evaluated = amplitude.EvaluateIntensity(normalization_mc);
    const std::vector<double> total_intensity(evaluated, evaluated + normalization_mc.Entries());
    ...
    for (std::size_t index = 0;
         ...
         ++index) {
        effective_yield +=
            backgrounds[index].likelihood_coefficient
            * (*backgrounds[index].sample).Entries();
    }
    ...
    for (int batch_begin = 0;
        ...
        const std::vector<double> packed_components =
            amplitude.EvaluateComponentBatch(normalization_mc,
    ...
    for (std::size_t sample_index = 0;
         sample_index < backgrounds.size();
         ++sample_index) {
        const GVVSample& background =
            (*backgrounds[sample_index].sample);
        ...
        weight_bg =
            -backgrounds[sample_index].likelihood_coefficient;
    ...
    double omega_resolution_sigma = amplitude.OmegaSigma();
