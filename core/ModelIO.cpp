...
// model document and records typed dynamics for active dependency compilation.
    ...
    const std::string& source_name)
{
    ...
    for (std::size_t index = 0; index < terms.size(); ++index) {
        ...
        term.dynamics.has_string_fields = dynamics.contains("type")
            && dynamics["type"].is_string() && dynamics.contains("resonance")
            && dynamics["resonance"].is_string();
        if (term.dynamics.has_string_fields) {
            term.dynamics.type = dynamics["type"].get<std::string>();
            term.dynamics.resonance = dynamics["resonance"].get<std::string>();
        }
        for (const auto& item : dynamics.items()) {
            if (item.key() != "type" && item.key() != "resonance")
                term.dynamics.unknown_fields.push_back(item.key());
        }
        if (term.active
