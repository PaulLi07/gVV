...
| Coherence class | Line style 2 (dashed), common width, one distinct color per class | All classes, including `0++`, `0-+`, and `2++`, use the same dashed style. |
...
The coherence-class palette must exclude black, gray, and the Total-fit blue.
When a model adds a class, extend the palette before colors repeat. Component
...
- The polarization figure uses three horizontal panels. Its single legend is
  placed in a compact strip spanning the canvas rather than in an empty fourth
  panel.
- The component figure uses six `3 x 2` physics panels plus a full-width,
  multi-column legend area. A large component list must never force the physics
  panels to shrink into a narrow `4 x 2` grid.
- Even angular moments use `2 x 2`; the ordered-omega odd diagnostic uses
  `3 x 1`.
- A legend is shared by panels with the same curve definitions. It must not
  obscure data, errors, peaks, or fitted curves. If a legend is kept inside a
  physics panel, that panel receives additional vertical headroom.
...
2. signed Background;
3. Total fit;
4. every coherence-class or Term-component curve in that panel.

After finding this envelope, the macro applies a small plot-specific headroom
factor. The omega-decay panel containing the shared legend deliberately uses a
larger upper factor. A style change must never clip a negative Background bin,
a component larger than the coherent Total, or the end of a data error bar.
...
Background. A coherence-class curve contains the coherent contribution within
that class. It does not display interference with other classes, so the visible
class curves need not sum to the Total fit.
...
8. Confirm that the component and coherence-class annotations describe omitted
