...
const std::vector<GVVWaveMetadata>& gvv_wave_registry()
{
    ...
    // WaveRegistry.cuh; individual formulae stay in process/waves/. A
    // coherence class names a shared phase convention and never masks a
    // cross-Wave term in the intensity.
    static const std::vector<GVVWaveMetadata> registry = {
        {"gvv.scalar_00", "0++", "0^{++}(00)",
         "positive_parity", GVV_SCALAR_00},
        {"gvv.scalar_22", "0++", "0^{++}(22)",
         "positive_parity", GVV_SCALAR_22},

        {"gvv.pseudoscalar_11", "0-+", "0^{-+}(11)",
         "negative_parity", GVV_PSEUDOSCALAR_11},

        {"gvv.tensor_02_u1", "2++", "2^{++}:U^{(1)}_{02}",
         "positive_parity", GVV_TENSOR_02_U1},
        {"gvv.tensor_02_u2", "2++", "2^{++}:U^{(2)}_{02}",
         "positive_parity", GVV_TENSOR_02_U2},
        {"gvv.tensor_02_u3", "2++", "2^{++}:U^{(3)}_{02}",
         "positive_parity", GVV_TENSOR_02_U3},

        {"gvv.tensor_20_u1", "2++", "2^{++}:U^{(1)}_{20}",
         "positive_parity", GVV_TENSOR_20_U1},
        {"gvv.tensor_20_u2", "2++", "2^{++}:U^{(2)}_{20}",
         "positive_parity", GVV_TENSOR_20_U2},
        {"gvv.tensor_20_u3", "2++", "2^{++}:U^{(3)}_{20}",
         "positive_parity", GVV_TENSOR_20_U3},

        {"gvv.tensor_22_u1", "2++", "2^{++}:U^{(1)}_{22}",
         "positive_parity", GVV_TENSOR_22_U1},
        {"gvv.tensor_22_u2", "2++", "2^{++}:U^{(2)}_{22}",
         "positive_parity", GVV_TENSOR_22_U2},
        {"gvv.tensor_22_u3", "2++", "2^{++}:U^{(3)}_{22}",
         "positive_parity", GVV_TENSOR_22_U3},

        {"gvv.tensor_42_u1", "2++", "2^{++}:U^{(1)}_{42}",
         "positive_parity", GVV_TENSOR_42_U1},
        {"gvv.tensor_42_u2", "2++", "2^{++}:U^{(2)}_{42}",
         "positive_parity", GVV_TENSOR_42_U2},
        {"gvv.tensor_42_u3", "2++", "2^{++}:U^{(3)}_{42}",
         "positive_parity", GVV_TENSOR_42_U3}
    };
    return registry;
}
