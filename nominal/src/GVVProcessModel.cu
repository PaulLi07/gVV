...
GVVResonanceParameters compile_resonance(
    const ctpwa::ResonanceDefinition& input)
{
    ...
    if (input.propagator == "fixed_width_bw") {
        return GVVResonanceParameters(
            GVV_PROP_FIXED_BW, mass.value, width.value, 0.0, 0);
    }
...
const std::vector<GVVWaveMetadata>& gvv_wave_registry()
{
    ...
    // device tensor dispatch remains in GVVAmplitude.h next to the formulae.
