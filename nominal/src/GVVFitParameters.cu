    ...
    GVVCompiledModel model)
{
    ...
    result.compiled_model = std::move(model);
    result.parameter_names =
        gvv_fit_parameter_names(result.compiled_model);
    result.errors.assign(result.parameter_names.size(), 0.0);

    const int number_terms =
        static_cast<int>(result.compiled_model.terms.size());
    const int number_resonances =
        static_cast<int>(result.compiled_model.resonances.size());
    std::vector<double> coupling_error_real(number_terms, 0.0);
    std::vector<double> coupling_error_imag(number_terms, 0.0);
    std::vector<double> coupling_error_log(number_terms, 0.0);
    std::vector<double> sd_error_log(number_resonances, 0.0);
    std::vector<double> flatte_error_log(number_resonances, 0.0);
    ...
    while (std::getline(input, line)) {
        ...
        if (tokens[0] == "parameter") {
            ...
            machine_rows[tokens[2]] = std::make_pair(
                parse_number(tokens[3], tokens[2]),
                parse_number(tokens[4], tokens[2] + " error"));
            continue;
        }
        if (tokens[0] == "coupling" && tokens.size() >= 4) {
            const int term = result.compiled_model.find_term(tokens[1]);
            if (term < 0) {
                throw std::runtime_error("unknown coupling " + tokens[1]);
            }
            result.compiled_model.initial_couplings[term] = DeviceComplex(
                parse_number(tokens[2], tokens[1]),
                parse_number(tokens[3], tokens[1]));
            if (tokens.size() >= 6 && tokens[4] != "fixed") {
                coupling_error_real[term] =
                    parse_number(tokens[4], tokens[1] + " Re error");
                coupling_error_imag[term] =
                    parse_number(tokens[5], tokens[1] + " Im error");
            }
            const int log_error = token_after(tokens, "log_error");
            if (log_error >= 0) {
                coupling_error_log[term] = parse_number(
        ...
        if (tokens[0] == "resonance" && tokens.size() >= 2) {
            const int resonance =
                result.compiled_model.find_resonance(tokens[1]);
            if (resonance < 0) {
                throw std::runtime_error("unknown resonance " + tokens[1]);
            }
            GVVResonanceParameters& state =
                result.compiled_model.resonances[resonance];
            ...
            if (log_error >= 0) {
                ...
                if (state.fit_sd_ratio) {
                    sd_error_log[resonance] = error;
                } else if (state.fit_flatte_ratio) {
                    flatte_error_log[resonance] = error;
                }
            }
        }
    }
    ...
    if (!machine_rows.empty()) {
        if (machine_rows.size() != result.parameter_names.size()) {
            throw std::runtime_error(
                "machine-readable parameter count does not match model");
        }
        result.values.resize(result.parameter_names.size());
        for (std::size_t index = 0;
             index < result.parameter_names.size();
             ++index) {
            const auto found = machine_rows.find(result.parameter_names[index]);
            if (found == machine_rows.end()) {
                throw std::runtime_error(
                    "missing machine-readable parameter "
                    + result.parameter_names[index]);
            }
            result.values[index] = found->second.first;
            result.errors[index] = found->second.second;
        }
        gvv_apply_fit_parameters_to_model(
            result.compiled_model, result.values);
        result.used_machine_readable_rows = true;
        return result;
    }

    result.values = gvv_fit_parameters_from_model(result.compiled_model);
    const std::vector<GVVFitParameterSpec> layout =
        gvv_fit_parameter_layout(result.compiled_model);
    for (std::size_t index = 0; index < layout.size(); ++index) {
        const GVVFitParameterSpec& parameter = layout[index];
        if (parameter.target == GVVFitParameterTarget::CouplingReal) {
            result.errors[index] = coupling_error_real[parameter.target_index];
        } else if (
            parameter.target == GVVFitParameterTarget::CouplingImaginary) {
            result.errors[index] = coupling_error_imag[parameter.target_index];
        } else if (
            parameter.target == GVVFitParameterTarget::CouplingLogMagnitude) {
            result.errors[index] = coupling_error_log[parameter.target_index];
        } else if (
            parameter.target == GVVFitParameterTarget::ResonanceLogSDRatio) {
            result.errors[index] = sd_error_log[parameter.target_index];
        } else {
            result.errors[index] = flatte_error_log[parameter.target_index];
        }
    }
    return result;
}
